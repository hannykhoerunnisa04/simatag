...
<body class="flex bg-gray-100 font-sans">
  ...
  <main class="md:ml-64 flex-1 p-6 md:p-8 lg:p-10 w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
      {{-- Diperbaiki: Nama rute disesuaikan --}}
            ...
            <tr class="hover:bg-blue-50">
              ...
              <td class="p-3 text-center">
                @if(strtolower($pengguna->role) == 'admin')
                  <span class="px-2 py-1 text-xs font-bold leading-tight rounded-full bg-indigo-100 text-indigo-700 uppercase">{{ $pengguna->role }}</span>
                @elseif(strtolower($pengguna->role) == 'atasan')
                  <span class="px-2 py-1 text-xs font-bold leading-tight rounded-full bg-purple-100 text-purple-700 uppercase">{{ $pengguna->role }}</span>
                @else
                  <span class="px-2 py-1 text-xs font-bold leading-tight rounded-full bg-gray-200 text-gray-700 uppercase">{{ $pengguna->role }}</span>
                @endif
              </td>
              ...
              <td class="p-3 text-center">
                <div class="flex items-center justify-center gap-4">
                  {{-- Diperbaiki: Nama rute disesuaikan --}}
                  <a href="{{ route('admin.pengguna.edit', $pengguna) }}" class="text-yellow-500 hover:text-yellow-700" title="Edit"><i class="fas fa-edit"></i></a>
