...
<body class="flex bg-gray-100 font-sans">
  ...
  <main class="md:ml-64 flex-1 p-6 md:p-8 lg:p-10 w-full">
    ...
    <!-- Tombol Aksi -->
    <div class="mb-6">
      {{-- Tombol create disesuaikan dengan route yang benar nanti --}}
      <a href="{{ route('admin.pemutusan.create') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200">
        ...
        <tbody class="text-gray-700 divide-y divide-gray-200">
          @forelse ($pemutusans ?? [] as $item)
            <tr class="hover:bg-blue-50">
              ...
              <td class="p-3">{{ \Carbon\Carbon::parse($item->tgl_pemutusan)->format('d F Y') }}</td>
              ...
              <td class="p-3 text-center">
                @if(strtolower($item->status_pemutusan) == 'permanen')
                  <span class="px-2 py-1 text-xs font-bold leading-tight rounded-full bg-red-100 text-red-700 uppercase">{{ $item->status_pemutusan }}</span>
                @elseif(strtolower($item->status_pemutusan) == 'sementara')
                  <span class="px-2 py-1 text-xs font-bold leading-tight rounded-full bg-yellow-100 text-yellow-700 uppercase">{{ $item->status_pemutusan }}</span>
                @else
                  <span class="px-2 py-1 text-xs font-bold leading-tight rounded-full bg-green-100 text-green-700 uppercase">{{ $item->status_pemutusan }}</span>
                @endif
              </td>
              <td class="p-3 text-center">
                <div class="flex items-center justify-center gap-4">
                  {{-- Diperbaiki: Link Edit dihubungkan ke rute --}}
                  <a href="{{ route('admin.pemutusan.edit', $item) }}" class="text-yellow-500 hover:text-yellow-700" title="Edit"><i class="fas fa-edit"></i></a>
                  <form action="{{ route('admin.pemutusan.destroy', $item) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus data ini?');">
                      @csrf
                      @method('DELETE')
