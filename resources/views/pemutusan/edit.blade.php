...
<head>
  ...
  <title>Edit Data Pemutusan</title>
...
<body class="flex bg-gray-100 font-sans">
  ...
  <main class="md:ml-64 flex-1 p-6 md:p-8 lg:p-10 w-full">
    ...
    <div class="bg-white shadow-xl rounded-lg p-6 md:p-8">
      ...
      <form action="{{ route('admin.pemutusan.update', $pemutusan) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6 max-w-lg mx-auto">
            <div>
              ...
              <input type="text" value="{{ $pemutusan->id_pemutusan }}" class="bg-gray-200 cursor-not-allowed border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5" readonly>
            </div>

            <div>
              <label for="id_pelanggan" class="block mb-2 text-sm font-medium text-gray-900">Pelanggan</label>
              <input type="text" value="{{ $pemutusan->pelanggan->nama_pelanggan ?? 'N/A' }} (ID: {{ $pemutusan->id_pelanggan }})" class="bg-gray-200 cursor-not-allowed border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5" readonly>
            </div>
            
            <div>
              <label for="tgl_pemutusan" class="block mb-2 text-sm font-medium text-gray-900">Tanggal Pemutusan</label>
              <input type="date" id="tgl_pemutusan" name="tgl_pemutusan" value="{{ old('tgl_pemutusan', $pemutusan->tgl_pemutusan) }}" class="bg-gray-50 border @error('tgl_pemutusan') border-red-500 @else border-gray-300 @enderror text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
              @error('tgl_pemutusan')
