    ...
    <div class="bg-white shadow-xl rounded-lg p-6 md:p-8">
      ...
      <form action="{{ route('admin.pemutusan.store') }}" method="POST">
        ...
        <div class="space-y-6 max-w-lg mx-auto">
            ...
            {{-- ID Pemutusan otomatis --}}
            <div>
              <label for="id_pemutusan" class="block mb-2 text-sm font-medium text-gray-900">ID Pemutusan (Otomatis)</label>
              <input type="text" id="id_pemutusan" name="id_pemutusan" value="{{ $newId }}" readonly
            ...
            <div>
              ...
              <input type="date" id="tgl_pemutusan" name="tgl_pemutusan" value="{{ old('tgl_pemutusan', date('Y-m-d')) }}"
                class="bg-gray-50 border @error('tgl_pemutusan') border-red-500 @else border-gray-300 @enderror text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
              @error('tgl_pemutusan')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
              @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="alasan_pemutusan" name="alasan_pemutusan" rows="4"
                  class="bg-gray-50 border @error('alasan_pemutusan') border-red-500 @else border-gray-300 @enderror text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Contoh: Menunggak pembayaran 3 bulan">{{ old('alasan_pemutusan') }}</textarea>
        ...
        <div class="mt-8 flex justify-end gap-4">
          <a href="{{ route('admin.pemutusan.index') }}" class="py-2.5 px-5 text-sm font-medium text-gray-900 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700">
            Batal
          </a>
